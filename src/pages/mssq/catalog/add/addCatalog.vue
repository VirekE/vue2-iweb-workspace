      ...
      <div class="tab-box">
        <el-table :data="catalogList" style="width: 100%;" height="100%" class="qz-table-hide">
          ...
          <el-table-column label="操作" width="100" fixed="right">
            <template slot-scope="scope">
              <!--只有审核不通过和草稿有编辑-->
              <el-button type="text" @click="editMinor(scope.row.id,scope.row.catalogId)">编辑
              </el-button>
              <el-button type="text"  @click="reMoveMinor(scope.row.id)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    onQuery() {
      var url = this.Config.baseContext + "/check/mssqCheckCatalog/selectCatalogList";
    ...
    reMoveMinor(id) {
      this.$confirm('是否确认删除该项目录?', '提示', {
        ...
        let url = this.Config.baseContext + "/check/mssqCheckCatalog/statusToDelete";
        this.$http_post(url, {
          catalogVid: id
        }).then(ret => {
          if (ret.state == 1) {
