...
<!--            <el-table-column prop="proCode" label="省事项编码" show-overflow-tooltip></el-table-column>-->
            ...
            <el-table-column prop="isReport" label="是否自动上报" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.isReport" active-value="1" inactive-value="0" active-text="是" inactive-text="否" @change="changeIsReport($event,scope.row,scope.$index)"></el-switch>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="right" width="240">
              <template slot-scope="scope">
                <el-button type="text" v-if="scope.row.status == '1' || (scope.row.status == '3' && scope.row.failNum <= '3')" @click="escalation(scope.row)">推送队列</el-button>
                <el-button type="text" v-if="scope.row.status == '3'" @click="handleError(scope.row)">最新上报详情</el-button>
                <el-button type="text" @click="viewHistory(scope.row)">查看历史</el-button>
              </template>
            </el-table-column>
          </el-table>
          <div class="block">
...
export default {
  ...
  methods: {
    onQuery(pageParam, searchParam) {
      var url = this.Config.baseContext + "/mssq/mssqEtlItem/selectReportList";
    ...
    searchClick(val) {
      this.formInline = {
        ...
        proName: val.proName.value,
        proCode: val.proCode.value,
    ...
    escalation(row){
      this.$confirm('是否上报事项【'+row.itemName+'】', '提示', {
        ...
        this.$http_post(delPro, {
          ...
          if (ret.state == 1) {
            this.$message.success('推送队列成功');
            var pageParam = this.itemParams;
            var searchParam = this.formInline;
            this.onQuery(pageParam, searchParam);
          }else {
            this.$message.error('推送队列失败')
          }
        })
      })
    },
