  ...
  <div class="material" ref="tableH">
    <div>
        ...
        <el-table :data="appLyList" style="width: 100%;" :height="tableHeight" class="qz-table-hide">
          ...
          <el-table-column label="操作" align="right" width="200" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button type="text" size="small" @click="editApply('todo',scope.row)">处理</el-button>
              <el-button type="text" size="small" v-if="scope.row.manageRole !== 'ZSJBB'" @click="deliver(scope.row)">转交</el-button>
              <el-button type="text" size="small" @click="dataGoBack(scope.row)">退回</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="block">
      ...
      <el-dialog :visible="deptDialog"
                 title="选择转交部门" width="40%"
                 @close="closeDept" class="qz-dialog-padding">
        <choose-dept :infoData="infoData" @pushDept="pushDept"></choose-dept>
      </el-dialog>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    onQuery() {
      var url = this.Config.baseContext + "/mssq/mssqItemDissent/itemDissentList";
    ...
    dataGoBack(row){
      ...
      option.show({
        ...
        onOkClick: function(res){
          ...
          let url = that.Config.baseContext + '/mssq/mssqItemDissent/dissentAuditGoBack';
          that.$http_post(url, {
            dissentId:row.id,
            remark:res
          }).then(ret => {
    ...
    pushDept(row,val){
     let params = {
        "dissentId": this.infoData.id,
        "itemCode": this.infoData.itemCode,
        "manageRole": val.role,
        "regionCode": val.regionCode,
       "disposalDeptId": row.id,
       "disposalDeptName": row.name,
       "disposalDeptNo": row.creditCode,
      }
      const rLoading = this.openLoading();
      let url = this.Config.baseContext+'/mssq/mssqItemDissent/dissentEdit';
      this.$http_post(url, params,true).then(ret => {
        rLoading.close();
        if(ret.state === 1){
          this.$message.success("异议转交成功");
          this.closeDept();
          this.onQuery();
        }else{
          this.$message.error(ret.message);
